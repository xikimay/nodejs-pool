...
cd ~
git clone https://github.com/xikimay/nodejs-pool.git  # Change this depending on how the deployment goes.
cd /usr/src/gtest
sudo cmake .
sudo make
...
cd UnitedBitcoin
sudo git checkout
curl https://raw.githubusercontent.com/Snipa22/nodejs-pool/master/deployment/ub_daemon.patch | sudo git apply -v
sudo make -j$(nproc)
sudo cp ~/nodejs-pool/deployment/ub.service /lib/systemd/system/
sudo useradd -m ubdaemon -d /home/ubdaemon

sudo systemctl daemon-reload
sudo systemctl enable ub
sudo systemctl start ub
